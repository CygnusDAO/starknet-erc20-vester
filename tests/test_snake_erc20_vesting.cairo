...
use token_vesting::mock::snake_erc20::{ISnakeERC20MockDispatcher, ISnakeERC20MockDispatcherTrait};
...
fn test_total_supply() {
    let mock_token = deploy_mock_token();
    assert(mock_token.total_supply() == INITIAL_SUPPLY, 'wrong_total_supply');
    assert(mock_token.balance_of(admin()) == INITIAL_SUPPLY, 'wrong_admin_balance');
}
...
fn test_transfer() {
    ...
    assert(mock_token.balance_of(admin()) == INITIAL_SUPPLY / 2, 'wrong_admin_balance');
    assert(mock_token.balance_of(rando()) == INITIAL_SUPPLY / 2, 'wrong_rando_balance');
}
...
fn test_fund_token_vesting() {
    ...
    assert(mock_token.balance_of(token_vesting.contract_address) == INITIAL_SUPPLY, 'vester_didnt_receive_tokens');
    assert(mock_token.balance_of(admin()) == 0, 'wrong_admin_balance');
...
fn test_vests_tokens_gradually() {
    ...
    start_warp(CheatTarget::One(token_vesting.contract_address), (duration / 2).try_into().unwrap());
    {
        ...
        assert(mock_token.balance_of(vested_user()) == 10, 'incorrect_user_balance');
    ...
    start_warp(CheatTarget::One(token_vesting.contract_address), (duration + 1).try_into().unwrap());
    {
        ...
        assert(mock_token.balance_of(vested_user()) == 55, 'wrong_new_balance_55');
        ...
        assert(mock_token.balance_of(vested_user()) == 100, 'wrong_new_balance_100');
...
fn test_should_release_tokens_if_revoked() {
    ...
    assert(mock_token.balance_of(vested_user()) == 50, 'beneficiary_no_receive_tokens')
}
...
fn deploy_mock_token() -> ISnakeERC20MockDispatcher {
    /// Declare mock
    let contract = declare('SnakeERC20Mock');
    ...
    ISnakeERC20MockDispatcher { contract_address }
}

fn deploy_mock_and_token_vesting() -> (ISnakeERC20MockDispatcher, ITokenVestingDispatcher) {
    /// Deploy token
    let token = deploy_mock_token();
    /// Declare mock
    let contract = declare('SnakeTokenVesting');
...
fn fund_vesting_token(mock_token: ISnakeERC20MockDispatcher, token_vesting: ITokenVestingDispatcher) {
    /// Transfer all supply to the vester (1000 tokens)
    start_prank(CheatTarget::One(mock_token.contract_address), admin());
    mock_token.transfer(token_vesting.contract_address, INITIAL_SUPPLY);
    stop_prank(CheatTarget::One(mock_token.contract_address));
